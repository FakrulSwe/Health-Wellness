		...
		<div class=""> 
			<form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;background-color: #e3e8ef;border: 1px #e3e8ef;border-radius: 5%; opacity:0.9;filter: alpha(opacity=60);text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" >
			  <div class="form-group center">
				<label for="user" style="font-weight: bold;color: #000000;">Hospital Name:</label>
				<input type="text" class="form-control" id="user" name="HospName" style="width:50%;margin-left: 24%" required>
			  </div>
			  <div class="form-group center">
				<label for="email" style="font-weight: bold;color: #000000;">Email:</label>
				<input type="email" class="form-control" id="email" name="Email" style="width:50%;margin-left: 24%" required>
			  </div>
			  ...
			  <div class="form-group">
				...
				<input type="text" class="form-control" id="mbl" pattern="[0-1]{2}[0-9]{9}" name="Mobile" style="width:50%;margin-left: 24%" required>
			  </div>
			  <div class="form-group">
				<label for="adrs" style="font-weight: bold;color: #000000;">Hospital Address:</label>
				<input type="text" class="form-control" id="adrs" name="HoAddress" style="width:50%;margin-left: 24%" required>
			  </div>
			  <div class="form-group">
				<label for="adrs" style="font-weight: bold;color: #000000;">Ambulance Number:</label>
				<input type="text" class="form-control" id="adrs" name="AmbNumber" style="width:50%;margin-left: 24%" required>
			  </div>
                <div class="form-group">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST"){
	            ...
	            $HMail=$_POST["Email"];
	            $AmMobile=$_POST["Mobile"];
	            $Address=$_POST["HoAddress"];
				$AmNumber=$_POST["AmbNumber"];
                $selected_val = $_POST['region'];  // Storing Selected Value In Variable
                
	            $conn=mysqli_connect("localhost","root","","Health&Wellness");
	               if(!$conn){
	           ...
	           $sql="insert into ambulogin(AmUserName,amPassword) values('".$HName."', '".$HPassword."')";
               $sql1="insert into ambulsignup(AmRegion,AmNumber,Mail,AmMobile,Address,HosName,Amid) values('".$selected_val."','".$AmNumber."','".$HMail."','".$AmMobile."','".$Address."', '".$HName."', LAST_INSERT_ID())";
