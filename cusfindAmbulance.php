...
<body class="cusfindphar">
    ...
    <div class="main-area">
        ...
        <div class="">
            <h2 class="text-center">Ambulance's Panel - Find Ambulance</h2>
                <br>
<!--                <label style="margin-left:40px;">User Name<br><?php echo $_SESSION["uname"];?></label>-->

                <br>
            <form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;" method="post" action=cusfindAmbulance2.php >
            ...
            <table style="background:white;border:1px solid black;" class="text-center">
                <thead>
                    <tr>
						...
						<th style="border:1px solid black">Ambulance Number</th>
...
//                        $str1=$_POST["search"];
                        ...
                        //$row = mysqli_fetch_array($result);
                         $result = mysqli_query($con,"SELECT * FROM ambulsignup;") or die("Failed to fetch".mysql_error());
                             
                             if( mysqli_num_rows( $result)==0 ){
                             $result1 = mysqli_query($con,"SELECT * FROM ambulsignup where HosName='".$str1."';") or die("Failed to fetch".mysql_error()); 
                                if( mysqli_num_rows( $result1)==0 ){
                                    ...
                                    while( $row = mysqli_fetch_assoc( $result1) ){
                                        echo "<tr><td>{$row['HosName']}</td><td>{$row['AmMobile']}</td><td>{$row['Address']}</td><td>{$row['Mail']}</td><td>{$row['AmNumber']}</td><td>{$row['AmRegion']}</td></tr>\n";
                                }
                                }
                             }
                        else{
